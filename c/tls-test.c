...
/*
This is a simple test program to retrieve files over HTTP/1.0 + TLS
...
./configure \
  ...
  --enable-static --enable-tls13 --disable-oldtls --enable-sni \
  --enable-altcertchains --disable-asm --enable-fastmath
...
make tls-test
./tls-test neon.tech 443
./tls-test expired.badssl.com 443 /  # see badssl.com for other testing domains
...
int main(int argc, char** argv) {
  ...
  do {
    ret = wolfSSL_read(ssl, buff,
                       sizeof(buff) - 1 /* leave space for null-termination */);
    if (ret == -1) {

...
Module.onRuntimeInitialized = function () {
  const handshake = Module.cwrap('handshake', 'number', ['string'], { async: true });
  ...
  function dequeueIncomingData() {
    ...
    for (let i = 0; i < len; i++) Module.setValue(globalBuff + i, nextData[i], 'i8');

    const resolve = globalResolve;
    globalResolve = globalBuff = null;
    globalMaxSize = 0;
  ...
  Module.provideEncryptedFromNetwork = (buff, maxSize) => {
  ...
  Module.receiveDecryptedFromLibrary = (buff, size) => {
    console.log(`receiving ${size} decrypted bytes from library`);

    console.log(buff);
  }

  Module.writeEncryptedToNetwork = (buff, size) => {
    console.log(`writing ${size} encrypted bytes to network`);

    const arr = byteArrayFromPointer(buff, size);
    socket.sendb(arr);
    return size;
  }
  ...
  socket.onOpen = async () => {
    ...
    let result = await handshake(host);
    console.log(result);
